...
function Empty({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="empty"
      className={cn(
        "flex min-h-48 flex-col items-center justify-center gap-section rounded-lg border border-dashed bg-card/50 p-region text-center",
        className,
      )}
      {...props}
    />
...
function EmptyIcon({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "flex size-12 items-center justify-center rounded-full bg-secondary text-secondary-foreground [&>svg]:size-6 [&>svg]:text-current",
        className,
      )}
      {...props}
    />
