...
Presentational primitives for surfacing downloadable documents (rulesets, guides, PDFs, attachments). No fetching — parents supply `href`s. The consumer owns the section header (title / description); these primitives render only the items.

## When to use which

| Need | Use |
| --- | --- |
| One document inside an existing list or section | `DownloadableItem` (`variant="row"`, default) |
| One document as a standalone tile | `DownloadableItem` (`variant="card"`) |
| Multiple documents as a vertical list of rows | `DownloadableItemList` |
| Multiple documents in a responsive 1 / 2 / 3-column grid | `DownloadableItemGrid` |
...
- **`card`** — file icon top-left, title + description on top, format hint bottom-left, download affordance bottom-right. **Always stacked** (no breakpoint switching). Cards fill their grid track (no internal width cap) — when used inside `DownloadableItemGrid`, the grid's stepped column count handles per-card sizing. Pick this when documents need more visual presence as discrete tiles — feature lists, knowledge-base previews, attachment galleries. When no `onDelete` is passed, the **entire card becomes the download anchor** (richer hit target, single click affordance).

## `DownloadableItemList`

Vertical list of `variant="row"` items inside an `ItemGroup`. Use when documents are presented as a simple stacked list under a section header.
...
The grid behavior is backed by the `card-grid` Tailwind utility (defined in `packages/ui/src/styles/globals.css`). The utility requires an ancestor with `container-type: inline-size` (Tailwind's `@container` class) — `DownloadableItemGrid` provides this wrapper automatically. If you use `card-grid` directly on another card-style surface, wrap it in `<div class="@container">` yourself.

## Props

- **`DownloadableItemData`** — `id`, `title`, optional `description`, `date`, `formatHint`, `href`.
