...
- **`stepper` / `stepperPosition`:** Pass any step UI (e.g. `StepLayoutStepper` or the ReUI `Stepper` from `@procertus-ui/ui`). `top` places it full width above the title; `start` uses a start rail (e.g. vertical stepper) beside title + body + footer from `md` up, stacking on small viewports.
