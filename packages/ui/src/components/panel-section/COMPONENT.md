...
- **`packages/ui`**: presentational **section stack**, optional **`h2` title**, optional **description** (`<p>`, muted), then **`children`**.
- **Spacing:** title and description sit flush; **`gap-section`** between the intro block and the body (design tokens).
...
3. **Exports**: `index.ts` and root `packages/ui/src/index.ts`.
