...
This template **names private pieces** in the same file (`MilestoneTimelineAlertBand`, `MilestoneTimelineHero`) to show how to split UI before extracting files. When a slice stabilizes, move it to `MilestoneTimeline-alert-band.tsx` (or similar) and re-export from this folder.
...
1. **Presentation** — `MilestoneTimeline.tsx` only; no data fetching.
2. **Stories** — Add variants (loading, error, empty) as separate stories.
3. **State** — Add a `useMilestoneTimeline` hook in this folder if you need state, then export it from `index.ts`.
