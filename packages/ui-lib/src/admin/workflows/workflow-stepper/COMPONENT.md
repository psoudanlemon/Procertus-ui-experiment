...
This template **names private pieces** in the same file (`WorkflowStepperAlertBand`, `WorkflowStepperHero`) to show how to split UI before extracting files. When a slice stabilizes, move it to `WorkflowStepper-alert-band.tsx` (or similar) and re-export from this folder.
...
1. **Presentation** — `WorkflowStepper.tsx` only; no data fetching.
2. **Stories** — Add variants (loading, error, empty) as separate stories.
3. **State** — Add a `useWorkflowStepper` hook in this folder if you need state, then export it from `index.ts`.
