...
const config: StorybookConfig = {
  ...
  typescript: {
    ...
    reactDocgenTypescriptOptions: {
      // Paths relative to this file (`.storybook/`): `../src` = ui-certification, `../../ui/src` = primitives package.
      include: [
        path.join(packageRoot, "src/**/*.tsx").replace(/\\/g, "/"),
        path.join(uiPackageRoot, "src/**/*.tsx").replace(/\\/g, "/"),
        path.join(uiLibPackageRoot, "src/**/*.tsx").replace(/\\/g, "/"),
      ],
    },
  },
  async viteFinal(configStorybook) {
    ...
    return mergeConfig(configStorybook, {
      ...
      resolve: {
        dedupe: ["react", "react-dom", "@procertus-ui/ui", "@procertus-ui/ui-lib"],
      },
      build: {
