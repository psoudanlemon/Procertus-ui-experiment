...
/** Keep in sync with `OnboardingFlow.tsx`. */
export const ONBOARDING_FLOW_STORAGE_KEY = "pt1:onboarding-flow-state";
/** Keep in sync with `OnboardingFlow.tsx` / `authenticatedRequestStore`. */
...
/** Removes completion record, onboarding flow state, and certification draft store used by the onboarding flow. */
